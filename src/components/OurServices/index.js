import './index.css'

const OurServices = () => {
  console.log('our services')
  return (
    <div id="servicesSection" className="our-services-cont">
      <h1 className="services-heading">OUR SERVICES</h1>
      <div className="line-arrow-dot-ser">
        <p className="dot-ser"> </p>
        <hr className="line-ser" />
      </div>
      <ul className="services-list-cont">
        <li className="service-item">
          <h1 className="service-heading">EXPLAINER VIDEOS</h1>
          <hr className="service-line" />
          <p className="service-des">
            Unveil your brands potential with our captivating explainer videos.
            Engaging visuals, seamless animations, and strategic storytelling
            that educates and inspires action. Elevate your brand with Patco
            Productions. Let s bring your story to life.
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">MOTION GRAPHICS</h1>
          <hr className="service-line" />
          <p className="service-des">
            Captivating motion graphics that bring your brand to life. Enhance
            your content with dynamic visuals, engaging animations, and seamless
            transitions. Elevate your message with our expert motion graphics
            services.
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">3D/VFX</h1>
          <hr className="service-line" />
          <p className="service-des">
            Experience the power of 3D VFX. Mind blowing visual effects,
            realistic simulations, and immersive virtual worlds. Take your
            visuals to new dimensions with cutting edge 3D VFX
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">LIVE SHOOT</h1>
          <hr className="service-line" />
          <p className="service-des">
            Immersive live shoots that capture the essence of the moment.
            Expertly crafted visuals, dynamic camera work, and captivating
            storytelling. Experience the magic of live shoots with us.
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">VIDEO EDITING</h1>
          <hr className="service-line" />
          <p className="service-des">
            Precise video editing that brings your story to life. Enhance your
            footage, add captivating effects, and deliver a polished final
            product. Elevate your videos with our expert editing services.
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">CONTENT WRITING</h1>
          <hr className="service-line" />
          <p className="service-des">
            Compelling content that captivates and engages. From website copy to
            blog posts, we craft words that leave a lasting impression. Ignite
            your brand with our expert content writing.
          </p>
        </li>
        <li className="service-item">
          <h1 className="service-heading">SOCIAL MEDIA CREATIVES CREatives</h1>
          <hr className="service-line" />
          <p className="service-des">
            Engaging social media content that sparks conversations and drives
            results. Stand out, connect with your audience, and make an impact
            with our strategic social media solutions.
          </p>
        </li>
      </ul>
    </div>
  )
}

export default OurServices
